...
FaceAgeDetNode::FaceAgeDetNode(const std::string &node_name, const NodeOptions &options) : DnnNode(node_name, options)
{
    ...
    {
        //         predict_task_ = std::make_shared<std::thread>(std::bind(&HandLmkDetNode::RunPredict, this));

        //         ai_msg_manage_ = std::make_shared<AiMsgManage>();

        //         RCLCPP_INFO(rclcpp::get_logger("hand_lmk_det"), "ai_msg_pub_topic_name: %s", ai_msg_pub_topic_name.data());
        //         msg_publisher_ = this->create_publisher<ai_msgs::msg::PerceptionTargets>(ai_msg_pub_topic_name, 10);

        //         RCLCPP_INFO(rclcpp::get_logger("hand lmk ai msg sub"), "Create subscription with topic_name: %s", ai_msg_sub_topic_name_.c_str());
        //         ai_msg_subscription_ = this->create_subscription<ai_msgs::msg::PerceptionTargets>(ai_msg_sub_topic_name_, 10, std::bind(&HandLmkDetNode::AiMsgProcess, this, std::placeholders::_1));

        //         if (is_shared_mem_sub_)
        //         {
        // #ifdef SHARED_MEM_ENABLED
        //             RCLCPP_WARN(rclcpp::get_logger("hand_lmk_det"), "Create hbmem_subscription with topic_name: %s", sharedmem_img_topic_name_.c_str());
        ...
        //                                                                                                     std::bind(&HandLmkDetNode::SharedMemImgProcess, this, std::placeholders::_1));
        // #else
        ...
        //         else
        //         {
        //             RCLCPP_WARN(rclcpp::get_logger("hand_lmk_det"), "Create subscription with topic_name: %s", ros_img_topic_name_.c_str());
